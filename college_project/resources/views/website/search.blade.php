...
@section('website-content')
    ...
    <section>
        <div class="product-section custom-container">
            <h4 class="product-section-item">
                              @foreach ($category as  $item22)
                                    @if(in_array($item22->id, $category_array))
                                        <label for="brand{{ $item22->id }}"> {{ $item22->name }} ></label>
                                        @break
            ...
            <div class="row">
                <div class="col-md-2" id="product-left-side">
                        ...
                        <div class="display-block-sm">
                            <div class="accordion mb-2" id="accordionExample">
                                ...
                                <h2 class="accordion-header" id="headingThree">
                                    <button class="accordion-button custom-accordion collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseThree" aria-expanded="false" aria-controls="collapseThree">
                                    Filter
                                    </button>
                                </h2>
                                        ...
                                        <div class="product-filter-section">
                                            ...
                                            <div class="brand">
                                                @foreach ($brands as $item)
                                                    @if(in_array($item->id, $brand_array))
                                                    <input type="checkbox" id="{{ $item->id }}" name="brand" class="sort-form" value="{{$item->id}}">
                                                    <label for="brand{{ $item->id }}"> {{ $item->name }}</label><br>
                                                    @endif
                                                @endforeach
                                            ...
                                            <div class="filter-title">Category</div>
                                            <div class="category">
                                                @foreach ($category as $item23)
                                                    @if(in_array($item23->id, $category_array))
                                                        <input type="checkbox" id="{{ $item23->id }}" name="category" class="sort-form" value="{{$item23->id}}">
                                                        <label for="brand{{ $item23->id }}"> {{ $item23->name }}</label><br>
                                                    @endif
                                                
                                                    {{-- <input type="checkbox" id="{{ $item->id }}" class="sort-form" name="cat" value="{{$item->id}}">
                                                    <label for="cat{{ $item->id }}"> {{ $item->name }}</label><br> --}}
                                                @endforeach
                                            </div>
                                            {{-- <div class="filter-title">Recording Modes</div>
                    ...
                    <!-- close for mobile device -->
                    
                </div>
                <div class="product-left col-md-10">
                    <div class="product-collaps-btn-part d-flex py-3">
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            $(".filter-form").on("change", ".sort-form", function(e){
                ...
                var url = '{{ url('product-short') }}/'+ "<?php echo $_GET["q"] ?>";
                $.ajax({
                    url:url,
                    method:'get',
                    // data:data ,
